    ...
    @IBAction func submitAction(_ sender: Any) {
        ...
        if confirmpasswordTF.text == passwordTF.text {
            if user._id != nil{
                ...
                } else if firstnameTF.text! == "" || lastnameTF.text! == "" || emailTF.text! == "" || phoneTF.text! == "" || locationTF.text! == "" {
                
                    let alert = UIAlertController(title: "Warning", message: "Please fill all the form",preferredStyle: .alert)
                    let action = UIAlertAction(title:"ok", style: .cancel, handler: nil)
                    alert.addAction(action)
                    self.present(alert, animated: true, completion: nil)
